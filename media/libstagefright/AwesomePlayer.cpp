...
void AwesomePlayer::onStreamDone() {
    ...
    if (mStreamDoneStatus != ERROR_END_OF_STREAM) {
        ALOGE("MEDIA_ERROR %d", mStreamDoneStatus);
...
void AwesomePlayer::shutdownVideoDecoder_l() {
    ...
    IPCThreadState::self()->flushCommands();
    ALOGI("video decoder shutdown completed");
}
...
status_t AwesomePlayer::initAudioDecoder() {
	...
	//if (mUsingMidwareAudioDecFlag == false) {
	if (0) { //cz_20121002 remove actdecoder
...
status_t AwesomePlayer::finishSetDataSource_l() {
    ...
    if (!strncasecmp("http://", mUri.string(), 7)
        ...
        if (mUIDValid) {
			 ALOGI("finishSetDataSource_l: mUIDValid: %d", mUIDValid);
...
ALOGE("mAudioSource stop end1 \n");
	 ...
	 modifyFlags(AUDIO_RUNNING, CLEAR);
        ...
        if(ret == NO_ERROR) {
            mTimeSource = NULL;
            //setAudioPlayer();
            //replace this gingerbread function, by samuel
        if (mAudioSource != NULL) {
                if (mAudioPlayer == NULL) {
                    if (mAudioSink != NULL) {
...
#if 0 
    if(setTrackAtPause){
	ALOGI("resume pause after setAudioTrack");
       pause_l();
    }
    if(tagPlaying) {
    	play_l();
